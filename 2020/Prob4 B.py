...
dataset = """eyr:2024 pid:662406624 hcl:#cfa07d byr:1947 iyr:2015 ecl:amb hgt:150cm
...
total = 0
checks = ["ecl", "byr", "eyr", "iyr", "hgt", "hcl", "pid"]
seq = []
...
for i in seq:
    realDataset.append(re.split('\n| ', i))
print(realDataset)
seq = []

for element in realDataset:
    print("Counter was reset ")
    counter = 0
    subcounter = 0
    for subElement in element:
        if subElement[:3] == "byr" and 1920 <= int(subElement[4:]) <= 2002:
            counter += 1
        elif subElement[:3] == "iyr" and 2010 <= int(subElement[4:]) <= 2020:
            counter += 1
        elif subElement[:3] == "eyr" and 2020 <= int(subElement[4:]) <= 2030:
            counter += 1
        elif subElement[:3] == "hgt":
            if subElement[-2:] == "cm" and '150' <= subElement[-5:-2] <= '193':
                counter += 1
            elif '59' <= subElement[-4:-2] <= '76':
                counter += 1
        elif subElement[:5] == "hcl:#":
            for i in range(5, 11):
                if subElement[i] in 'abcdef' or subElement[i] in '0123456789':
                    subcounter += 1
            if subcounter == 6:
                counter += 1
        elif subElement[:3] == "ecl" and subElement[4:] in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']:
            counter += 1
        elif subElement[:3] == "pid" and len(subElement[5:]) == 9:
            counter += 1
    print(counter)
    if counter == 7:
        total += 1
