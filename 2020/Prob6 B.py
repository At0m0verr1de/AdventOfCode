dataset = """rypdeiqkbgacnxwumhtozfjvs
...
for groups in dataset:
    realDataset.append(groups.split('\n'))

for group in realDataset:
    for i in range(0, len(group[0])):
        counter = 0
        elementToCheck = group[0][i]
        print(f"checking {elementToCheck}")
        for j in range(0, len(group)):
            if elementToCheck in group[j]:
                counter += 1
